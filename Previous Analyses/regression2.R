...
setwd('/proj/econ/duartema/GasBrazil/CodeStruct/')
...
n_competitors_3km <- apply(dist_matrix, 1, function(x) sum(x > 0 & x <= 1000, na.rm = TRUE))
...
reg2 <- feols(
  p_sell ~ n_competitors_3km | month + ra_num,
  data    = reg_sample,
  cluster = ~cl
)
