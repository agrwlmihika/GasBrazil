...
# ------------------------------------------------------------
# Load saved model objects from spatial regression
# ------------------------------------------------------------
load('/proj/econ/duartema/GasBrazil/Mihika_Practice/spatial_regression_NEW.RData')
load('/proj/econ/duartema/GasBrazil/Mihika_Practice/all_elasticities_NEW.RData')
...
# ------------------------------------------------------------
# UPDATED compute_elasticity_month (with sign fix)
# ------------------------------------------------------------
compute_elasticity_month <- function(m, Dmat, gas_m, g1, g2, g3, a) {
  ...
  dG_ds   <- gamma_w / S_sum          # sign fix
  for (j in seq_len(n)) {
    sk_over_sum    <- S / (S[j] + S)
    sk_over_sum[j] <- 0
    spatial_term   <- -sum((g1*w1[j,] + g2*w2[j,] + g3*w3[j,]) * sk_over_sum)  # sign fix
...
run_elasticity <- function(g1, g2, g3, alpha, model_name = "") {
  ...
  elas_list   <- vector("list", length(months_list))
  for (i in seq_along(months_list)) {
    if (i %% 5 == 0) cat("    Month", i, "of", length(months_list), "\n")
    m     <- months_list[i]
...
# ------------------------------------------------------------
# STEP 1: Compute elasticities (delete old file first to recompute)
# ------------------------------------------------------------
...
cat("=== Elasticity Medians ===\n")
cat("Own:     sOLS =", med_own_sOLS, " sIV1 =", med_own_sIV1, " sIV2 =", med_own_sIV2, "\n")
...
extra_rows <- data.frame(
  term = c("theta (market size)", "Median own elasticity",
           "Median cross (< 500m)", "Median cross (500m-1.5km)",
           "Median cross (1.5km-3km)", "First stage F-stat"),
...
modelsummary(
  ...
  output     = "/proj/econ/duartema/GasBrazil/Final/Final_Table_NewPriceInstruments.txt"
)